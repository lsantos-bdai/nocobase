 ...
 *    Returns a flat array of objects with $schema URLs.
 ...
 * - env: "prod" or "dev" — selects GCS bucket for $schema URLs (required)
 ...
 *
 * Response: { data: [{$schema, ...fields}], meta }
 */
export async function getSchemaConformant(ctx: Context, next: Next) {
  ...
  if (getRelations) {
    ...
    // Get collection titles for schema URL generation
    ...
    // Build flat array with $schema URLs in graph order
    ...
    for (const ref of windowRefs) {
      ...
      const collTitle = collectionTitles[ref.collection] || ref.collection;
      const schemaUrl = getSchemaUrl(responseType, collTitle);
    ...
    // Build flat array with $schema URLs
    const data: Record<string, unknown>[] = [];
