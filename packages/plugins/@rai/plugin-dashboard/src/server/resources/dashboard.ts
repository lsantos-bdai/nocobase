...
const T = () =>
  `\`${process.env.GCP_PROJECT_ID}.${process.env.BQ_DATASET}.${process.env.BQ_TABLE ?? 'sessions'}\``;
...
async function runQuery(sql: string, params: Record<string, any> = {}): Promise<any[]> {
...
export const actions = {
  ...
  async filterOptions(ctx: any) {
    const [collectors, robots, dataTypes] = await Promise.all([
      runQuery(
        `SELECT DISTINCT collector FROM ${T()} WHERE collector IS NOT NULL ORDER BY collector LIMIT 200`,
      ),
      runQuery(
        `SELECT DISTINCT robot_id FROM ${T()} WHERE robot_id IS NOT NULL ORDER BY robot_id LIMIT 200`,
      ),
      runQuery(
        `SELECT DISTINCT data_type FROM ${T()} WHERE data_type IS NOT NULL ORDER BY data_type LIMIT 100`,
      ),
    ]);
    ctx.body = {
